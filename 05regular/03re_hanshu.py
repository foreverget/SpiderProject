...
import re
# 1、match,从开始的位置进行匹配。如果开始的位置没有匹配到。就直接失败了。示例代码如下：
text = 'hello'
ret = re.match('h',text)
print(ret.group())
...
# 2、如果想要匹配换行的数据，那么就要传入一个flag=re.DOTALL，就可以匹配换行符了。示例代码如下：
text = "abc\nabc"
ret = re.match('abc.*abc',text,re.DOTALL)
print(ret.group())

#3、search,在字符串中找满足条件的字符。如果找到，就返回。说白了，就是只会找到第一个满足条件的。
...
# 5、sub,用来替换字符串。将匹配到的字符串替换为其他字符串。
text = 'apple price $99 orange price $88'
ret = re.sub('\d+','0',text)
print(ret)

# 6、sub函数的案例，获取拉勾网中的数据：
html = """
<div>
<p>基本要求：</p>
...
# 7、split,使用正则表达式来分割字符串。
text = "hello world ni hao"
ret = re.split('\W',text)
print(ret)


#8、compile,对于一些经常要用到的正则表达式，可以使用compile进行编译，后期再使用的时候可以直接拿过来用，执行效率会更快。
# 而且compile还可以指定flag=re.VERBOSE，在写正则表达式的时候可以做好注释。示例代码如下：
